...
def _watch(bot: Client, update: Message, args: list, isTar=False, isZip=False):
    try:
        link = args[0]
    except IndexError:
    ...
    uriadded = sendUriAdded(update, bot)
    sendMessage(f"{uriadded}", bot, update)
    if len(Interval) == 0:
...
@Client.on_message(
    filters.command(BotCommands.ZipWatchCommand) &
    filters.chat(AUTHORIZED_CHATS)
)
def watchZip(client: Client, message: Message):
    args = [" ".join(message.command[1:])]
    _watch(client, message, args, isZip=True)     